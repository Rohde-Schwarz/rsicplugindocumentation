...
.. image:: images/func_panel_macros.png
...
3. **Add Default** - adds universal macros that you can use with any instrument (for example, read instrument options). Use these as a starting point to define your own macro scripts.
4. **Add Existing** - copy the existing macros from other instruments.
5. **Macro Buttons Panel** - contains all the macros defined for your instrument. The context-menu allows you to open the folder containing the macro scripts. Also, each button has its own context-menu with related actions.

Use the button **3 - Add Default** and try out the macro **Get All Options**. When you start it, the script ``show_instrument_options.py`` executes initialization of your instrument, sends \*OPT? query and splits the response into B- and K- options.
