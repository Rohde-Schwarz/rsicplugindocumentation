...
Macros Panel provides quick way to perform custom actions with your instruments, like for example, reading instrument options, custom screenshots, special preset you need to execute often, or copying certain data from the instrument repeatedly.
...
In addition, the script gets the ``mods`` variable, that says which modification keys you used when clicking the macro button - CTRL/ALT/SHIFT. We have some ready-to-use macros (see button **3. Add Default**) to get you started quickly.

Let us look at the Function Panel:

.. image:: images/RsIcItwMacros-func_panel_macros.drawio.png

Description of the controls:

1. **Function Panel Macros** - tab selector for the Macros.
2. **Add Default Macros** - adds universal macros that you can use with any instrument (for example, read instrument options). Use these as a starting point to define your own macro scripts.
3. **Add new Macro Button** - shows a dialog where you specify the macro's name (button's label), script file (if new, it will be created and opened in your Pycharm editor), the function name, and the kwargs (optional).
...
Use the button **2 - Add Default Macros** and try out the macro **Get All Options**. When you start it, the script ``show_instrument_options.py`` executes initialization of your instrument, sends \*OPT? query and splits the response into B- and K- options.
...
Notice the ``__main__`` guard at the bottom - it helps you to debug the script, so you can run it as is. The parameters supplied are of the function panel from which you opened it, in our example an SMW200A instrument. The cool feature is, if you open the same macro script from another panel, for example an FSW, this part is updated to correspond the FSW settings. This way, if you debug the macro, you always have the correct parameters.

.. hint::
    The macros are stored to a folder which you can change in the :ref:`Settings -> Instrument Tool Windows <12.3 Instrument Tool Windows>`. If you want to edit a macro, you get a warning window from Pycharm, because the folder is not a part of you project. You can safely select the first or the second option:
...
Let us look at creating a new macro. Use the button **3 - Add New Macro**, fill out the Button Display Name, the Script Name and hit **OK**:

.. image:: images/RsIcItwMacros-func_panel_macros_add_new.drawio.png
